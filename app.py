...
try:
    ...
    print(
        f"Warning: Could not import core modules: {e}. Ensure blockchain.py and storage.py exist."
    )
...
def create_oracle(email, name, org_type, password):
    ...
    cursor.execute(
        "INSERT INTO oracles (email, name, org_type, password_hash, created_at) VALUES (?, ?, ?, ?, datetime('now'))",
...
def oracle_dashboard():
    ...
    return render_template(
        "oracle_dashboard.html", oracle_email=session.get("oracle_email")
    )
...
def oracle_verify():
    ...
    try:
        ...
        return jsonify(
            {"error": f"Failed to fetch IPFS metadata: {e}", "is_valid": False}
        ), 500
    ...
    try:
        ...
        if conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT tx_hash, ipfs_url FROM registry WHERE sha256 = ? OR ipfs_url = ? LIMIT 1",
                (sha, ipfs_url),
            )
            row = cur.fetchone()
    ...
    return jsonify(
        {"is_valid": is_valid, "matches_ipfs": matches_ipfs, "anchored": anchored}
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Warning: could not ensure registry table at startup: {e}")
    try:
        ensure_oracles_table_exists()
    except Exception as e:
        print(f"Warning: could not ensure oracles table at startup: {e}")
